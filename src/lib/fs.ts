 ...
 */
export function getDirectoryContents(directoryPath: string, basePath: string = ''): NavItem[] {
  const items: NavItem[] = [];
  ...
  for (const item of contents) {
    ...
    if (item.startsWith('.') || item === 'node_modules' || item === '_app') continue;
    ...
    if (stats.isDirectory()) {
      ...
      const hasIndex = fs.existsSync(path.join(fullPath, 'index.mdx')) || 
                       fs.existsSync(path.join(fullPath, 'index.jsx')) ||
                       fs.existsSync(path.join(fullPath, 'index.tsx'));
                       
      // Get child items
      const children = getDirectoryContents(fullPath, path.join(basePath, item));
      ...
      if (item !== 'index.mdx' && item !== 'index.jsx' && item !== 'index.tsx' && item !== 'index.md') {
        items.push({
          title: formatTitle(item),
          slug
        });
